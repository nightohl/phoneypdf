...
class BitWriter(object):
    ...
    def write(self, data, length):
        ...
        if not ( length >= 0 and (1 << length) > data ):
            raise Exception(BitWriterException, "Invalid data length")
...
class BitReader(object):
    ...
    def pos(self, bits):
        ...
        """
        if bits > self.size:
            raise Exception( BitReaderException, "Pointer position out of data" )
    ...
    def peek(self, length):
        ...
        """
        if length <= 0:
            raise Exception( BitReaderException, "Invalid read length")
        elif ( self.pos + length ) > self.size:
            raise Exception( BitReaderException, "Insufficient data")
    ...
    def decode(self, stream, k = 0, eol = False, byteAlign = False, columns = 1728, rows = 0, eob = True, blackIs1 = False, damagedRowsBeforeError = 0):
        ...
        while not ( bitr.eod_p or rows == 0 ):
            ...
            if bitr.peek(self.EOL[1]) != self.EOL[0]:
                if eol:
                    raise Exception( Exception, "No end-of-line pattern found (at bit pos %d/%d)" % (bitr.pos, bitr.size))
            else:
                bitr.pos += self.EOL[1]
            ...
            while line_length < columns:
                ...
                if bit_length == None:
                    raise Exception( Exception, "Unfinished line (at bit pos %d/%d), %s" % (bitr.pos, bitr.size, bitw.data))

                line_length += bit_length
                if line_length > columns:
                    raise Exception( Exception, "Line is too long (at bit pos %d/%d)" % (bitr.pos, bitr.size))
